...
namespace Application.Common.Interfaces.Notifications
{
    ...
    {
        Task SendUserNotificationAsync(Guid userId, string title, string message, NotificationType type = NotificationType.System);
        Task BroadcastAsync(IEnumerable<Guid> userIds, string title, string message, NotificationType type = NotificationType.System);
